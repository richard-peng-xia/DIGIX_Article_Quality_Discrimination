...
from Train_PU_model import ElkanotoPuClassifier
...
SUMMARY_OUTPUT_PATH = os.path.join("", "submission.csv")
...
def get_label_set_and_sample_num(config_path, sample_num=False):
    with open(config_path, "r", encoding="gbk", errors="ignore") as input_file:
...
def build_label_set_and_sample_num(input_paths, output_paths):
    ...
    with open(output_paths, "w", encoding="gbk", errors="ignore") as output_file:
