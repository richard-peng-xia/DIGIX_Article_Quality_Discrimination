"""
数据集预处理：对原始训练集，测试集进行简单的清洗，从训练集中输出带标记样本集(P)和未知样本集（U）
"""
import json
import os
...
# 获取数据集的标签集及其大小
def get_label_set_and_sample_num(dataset_path, sample_num=False):
    with open(dataset_path, 'r', encoding='UTF-8') as input_file:
        json_data = json.loads(input_file.readline())
        if sample_num:
            return json_data["label_set"], json_data["total_num"]
        else:
...
def build_label_set_and_sample_num(input_path, output_path):
    ...
    with open(output_path, 'w', encoding='UTF-8') as output_file:
        record = {"label_set": sorted(list(label_set)), "total_num": sample_num}
        json.dump(record, output_file, ensure_ascii=False)
        return record["label_set"], record["total_num"]
...
def check_duplicate_title(input_path, output_path):
    ...
    with open(input_path, "r", encoding='utf-8', errors='ignore') as inputfile, open(output_path, "w", encoding="utf-8", errors='ignore') as outfile:
...
def preprocess():
    ...
    # 对训练集中带标记的样本索引化标签
    index_data_pd(INDEX, PREPROCESSED_TRAIN_FILE_PATH, POSITIVE_TRAIN_FILE_PATH, UNLABELED_TRAIN_FILE_PATH)
